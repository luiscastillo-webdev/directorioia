...
const Plantilla = ({buscar}) => {  
  ...
  return (
    ...
    {buscar.map(({id,img,nombre,url,descrip,categoria,bgdestacado,pago})=>(
        <div key={id}
        className='mb-4 rounded overflow-hidden shadow-inner bg-blue-200'>
          ...
          >
          <img className="w-full cursor-pointer" src={`../../img/${img}.webp`} alt={nombre}/>
          </motion.div>
          <div className="px-6 py-4">
              <div className="mb-4"><h3 className='text-xl text-gray-900'>{nombre}</h3></div>
              <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{categoria}</span>
              <span className={`${bgdestacado} inline-block rounded-full px-3 py-1 text-sm font-semibold mr-2 mb-2`}>{pago}</span>
              <p className="justify-content text-gray-700">
              {descrip}
              </p>
          </div>
